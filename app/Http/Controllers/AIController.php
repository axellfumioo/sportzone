...
class AIController extends Controller
{
    ...
    public function AIRequest($token, $message)
    {
        ...
        $gokartAddresses = $arenaGokart->pluck('arena_address')->map(fn($address) => " $address")->join(",");
        $bowlingAddresses = $arenaBowling->pluck('arena_address')->map(fn($address) => " $address")->join(",");
        $billiardAddresses = $arenaBilliard->pluck('arena_address')->map(fn($address) => " $address")->join(",");
...
- Untuk panggilan ke user yaitu Kak, seperti contoh: Hai kak.
